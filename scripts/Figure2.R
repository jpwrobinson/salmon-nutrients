  ...
  bot<-ggplot(mn, aes(nutrient, nutrient_deficit)) + 
        ...
        geom_text(aes(y = nutrient_deficit_min, label = nutrient), hjust=1.15) +
        coord_flip() +
    scale_y_continuous(limits=c(-10, 200), breaks=seq(0, 200, 50)) +
    labs(x = '', y = 'wild nutrients retained in farmed salmon, %') + th +
          theme(axis.text.y =element_blank(), 
                axis.line.y = element_blank(), 
                plot.margin=unit(c(0,4,0,6), 'cm'))
